...
def auth_callback():
    ...
    if req != False:
        res = redirect(location="http://localhost:3000/auth")
        res.status_code = 302

        res.set_cookie("daystream_token", json.dumps(req), httponly=False, secure=False, samesite="Lax", domain="http://localhost:3000/auth")
